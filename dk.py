# =======================================================================
#  DocuMorph AI  –  all-in-one Streamlit app
#  Save this file (e.g. app.py) and run:  streamlit run app.py
# =======================================================================
...
class DocuMorphEngine:
    ...
    def add_logo(self, image_file: io.BytesIO | str, width, height):
        ...
        Works with a Streamlit-uploaded BytesIO or a file path.
        """
...
with tabs[0]:
    ...
    with c1:
        font_name = st.selectbox(
            "Font Style", ["Times New Roman", "Arial", "Calibri", "Georgia"], 0
        )
    ...
    with c2:
        alignment = st.radio(
            "Alignment", ["Left", "Center", "Right", "Justify"], horizontal=True
        )
        margins = [
            st.number_input(lbl, 0.1, 3.0, 1.0, 0.1, key=lbl)
            for lbl in (
                "Top Margin",
                "Bottom Margin",
                "Left Margin",
...
with tabs[3]:
    ...
    if st.button("📝 Generate & Download"):
        if not uploaded_file:
            ...
            if logo:
                eng.add_logo(logo, logo_w, logo_h)

            eng.set_header_footer(header_text, footer_text, hf_size, hf_align)

            if section_title.strip():
                eng.add_section_title(section_title.strip())
            ...
            if figure:
                figure.seek(0)
                eng.add_figure(figure, fig_w, fig_h, caption, caption_pos)
            ...
            os.unlink(tmp.name)  # delete the temp file after offering download
